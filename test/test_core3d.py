...
from auto_drone.sensing3d import RangeFrame, RangeMeasurement, generate_range_frame
...
def test_range_frame_is_sensor_like_not_direct_occupancy():
    ...
    assert frame.true_visible_cells
    assert all(measurement.range_voxels >= 0.0 for measurement in frame.measurements)


def test_mapper_weights_updates_by_reprojection_error():
    ...
    frame = RangeFrame(
        true_pose=Pose3D(2, 2, 2),
        estimated_pose=Pose3D(2, 2, 2),
        measurements=(
            RangeMeasurement(
                ray=((1, 0, 0), (2, 0, 0)),
                cells=((3, 2, 2), (4, 2, 2)),
    ...
    assert (3, 2, 2) in mapped
    assert (4, 2, 2) in mapped
    assert belief.occupancy_probability(3, 2, 2) < 0.5
    assert belief.occupancy_probability(4, 2, 2) > 0.5
